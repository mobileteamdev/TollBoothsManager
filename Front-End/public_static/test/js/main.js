...
function userBetSend() {
	...
	}).catch(function(e) {//alert(888)
	    //console.log("userBet: "+e);
	});
}
//------------------------------------------------------------------------------------------------------------------
function totalTollBooths() {
	contract.totalTollBooths().call().then(function(r) {
	    console.log("totalTollBooths: "+r);
	}).catch(function(e) {
	    console.log("totalTollBooths: "+e);
	});
}
function tollBooths(id) {
	contract.tollBooths(id).call().then(function(r) {
	    console.log("tollBooths: "+JSON.stringify(r));
	}).catch(function(e) {
	    console.log("tollBooths: "+e);
	});
}
function tollBoothsAdditional(id) {
	contract.tollBoothsAdditional(id).call().then(function(r) {
	    console.log("tollBoothsAdditional: "+JSON.stringify(r));
	}).catch(function(e) {
	    console.log("tollBoothsAdditional: "+e);
	});
}
function getTollBoothDetails(id) {
	contract.getTollBoothDetails(id).call().then(function(r) {
	    console.log("getTollBoothDetails: "+JSON.stringify(r));
	}).catch(function(e) {
	    console.log("getTollBoothDetails: "+e);
	});
}
function getTollBoothAdditional(id) {
	contract.getTollBoothAdditional(id).call().then(function(r) {
	    console.log("getTollBoothAdditional: "+JSON.stringify(r));
	}).catch(function(e) {
	    console.log("getTollBoothAdditional: "+e);
	});
}
function createNewTollBooth() {
	contract.createNewTollBooth("a","b","c",7,"d","e",9,true).send().then(function(r) {
...
function addRecentData(data, watchE){
	...
	if(watchE){
		$("#bet-modal-body").html("<p>Bet number: "+bet_number+"</p><p>Bet amount: "+bet_amount+" TRX</p><p>Result number: "+result_place+"</p><p>Win amount: "+win_amount+" TRX</p>");
		$("#mybetModal").modal("show");
	}
	//
	var html = '<div class="message-bet"><span class="owner">'+player+'</span> Bet number: <b>'+bet_number+'</b>, Bet amount: <b>'+bet_amount+' TRX</b>, Result number: <b>'+result_place+'</b>, Win amount: <b>'+win_amount+' TRX</b></div>';
